...
// ─── ✅ Approve purchase itself ───────────────────────────────────────────────
export const approvePurchase = createAsyncThunk(
  'purchases/approvePurchase',
  async (id, { rejectWithValue }) => {
    try {
      const response = await api.patch(`/admin/purchases/${id}/approve`);
      return response.data.data;
    } catch (err) {
      return rejectWithValue(err?.response?.data?.message || 'Failed to approve purchase');
    }
  }
);

// ─── ✅ Reject purchase itself ────────────────────────────────────────────────
export const rejectPurchase = createAsyncThunk(
  'purchases/rejectPurchase',
  async ({ id, rejectionReason }, { rejectWithValue }) => {
    try {
      const response = await api.patch(`/admin/purchases/${id}/reject`, { rejectionReason });
      return response.data.data;
    } catch (err) {
      return rejectWithValue(err?.response?.data?.message || 'Failed to reject purchase');
    }
  }
);