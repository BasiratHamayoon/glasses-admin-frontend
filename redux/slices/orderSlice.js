...
const removeItemFromList = (list, orderId) => {
...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(fetchWebsiteOrders.pending, (state) => {
    ...
    builder
      ...
      .addCase(fetchWebsiteOrderById.pending, (state) => {
        state.loadingDetails = true;
      })
    ...
    builder
      .addCase(approveCancellation.pending, (state) => {
        state.cancellationLoading = true;
      })
      .addCase(approveCancellation.fulfilled, (state, action) => {
        state.cancellationLoading = false;
        const { order, orderType } = action.payload;
        if (orderType === 'pos') {
          updateItemInList(state.pos.items, order);
        } else {
          updateItemInList(state.website.items, order);
        }
      })
      .addCase(approveCancellation.rejected, (state) => {
        state.cancellationLoading = false;
      })
      .addCase(rejectCancellation.pending, (state) => {
        state.cancellationLoading = true;
      })
      ...
      .addCase(rejectCancellation.rejected, (state) => {
        state.cancellationLoading = false;
      })
      .addCase(permanentlyDeleteOrder.pending, (state) => {
        state.cancellationLoading = true;
      })
      .addCase(permanentlyDeleteOrder.fulfilled, (state, action) => {
        state.cancellationLoading = false;
        const { orderId, orderType } = action.payload;
        if (orderType === 'pos') {
          state.pos.items = removeItemFromList(state.pos.items, orderId);
        } else {
          state.website.items = removeItemFromList(state.website.items, orderId);
        }
      })
      .addCase(permanentlyDeleteOrder.rejected, (state) => {
        state.cancellationLoading = false;
      });
  },
});
