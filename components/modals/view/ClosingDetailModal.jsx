  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (orders.length === 0) {
        ...
        const shopId = closingData.shop?._id;
      ...
      generateClosingReceipt(closingData);
    } finally {
    ...
    >
      {loading || !closingData ? (
        ...
        <div className="space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-neutral-50 dark:bg-[#0a0a0a] border border-neutral-200 dark:border-neutral-800 rounded-sm">
            <div className="min-w-0">
              <div className="text-lg font-black text-[#E9B10C] tracking-widest truncate">
                {closingData.closingNumber || "—"}
              </div>
              <div className="text-[10px] uppercase text-neutral-500 font-bold mt-1 truncate">
            ...
            <div className="flex items-center gap-2 flex-wrap shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#E9B10C]/10 text-[#E9B10C] text-[9px] uppercase font-black tracking-widest rounded-sm hover:bg-[#E9B10C]/20 transition-colors whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {printLoading ? (
              ...
              <span
                className={`px-3 py-1.5 text-[9px] uppercase tracking-widest font-black rounded-sm whitespace-nowrap ${
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <MoneyField label={t("actualCash")} value={closingData.actualCash} />
              <MoneyField label={t("actualCard")} value={closingData.actualCard} />
              <MoneyField label={t("actualUPI")} value={closingData.actualUPI} />
            </div>
          </div>
          ...
          {(closingData.totalSales !== undefined ||
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {closingData.totalSales !== undefined && (
                  <MoneyField label={t("totalSales")} value={closingData.totalSales} color="text-green-500" />
                )}
                {closingData.totalExpenses !== undefined && (
                  <MoneyField label={t("totalExpenses")} value={closingData.totalExpenses} color="text-red-500" />
                )}
                {closingData.netSales !== undefined && (
                  <MoneyField label={t("netSales")} value={closingData.netSales} color="text-blue-500" />
                )}
                {closingData.cashSales !== undefined && (
                  <MoneyField label={t("cashSales")} value={closingData.cashSales} />
                )}
                {closingData.cardSales !== undefined && (
                  <MoneyField label={t("cardSales")} value={closingData.cardSales} />
                )}
                {closingData.upiSales !== undefined && (
                  <MoneyField label={t("upiSales")} value={closingData.upiSales} />
                )}
                {closingData.systemClosingBalance !== undefined && (
                  <MoneyField label={t("systemBalance")} value={closingData.systemClosingBalance} />
                )}
                {closingData.physicalCash !== undefined && (
                  <MoneyField label={t("physicalCash")} value={closingData.physicalCash} />
                )}
                {closingData.hasVariance !== undefined && (
                  <Field
                    label={t("variance")}
                    value={
                      closingData.hasVariance
                        ? `${closingData.varianceType} (⃁ ${formatNum(closingData.varianceAmount)})`
                        : t("none")
                    }
                    color={
                      closingData.hasVariance
                ...
                {closingData.totalOrders !== undefined && (
                  <Field label={t("totalOrders")} value={String(closingData.totalOrders)} />
                )}
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {closingData.closedBy?.name && (
                <Field label={t("closedBy")} value={closingData.closedBy.name} />
              )}
              {closingData.approvedBy?.name && (
                <Field label={t("approvedBy")} value={closingData.approvedBy.name} />
              )}
            </div>
          </div>
          ...
          {["SUBMITTED", "PENDING", "OPEN"].includes(closingData.status) && (
            <div className="flex flex-col sm:flex-row gap-2 justify-end pt-2 border-t border-neutral-200 dark:border-neutral-800">
              {onApprove && (
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2.5 bg-green-500/10 text-green-500 text-[10px] uppercase font-black rounded-sm hover:bg-green-500/20 transition-colors w-full sm:w-auto"
                >
                  <CheckCircle size={13} />
                  {t("approve")}
                </button>
              ...
              {onReject && (
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2.5 bg-red-500/10 text-red-500 text-[10px] uppercase font-black rounded-sm hover:bg-red-500/20 transition-colors w-full sm:w-auto"
                >
                  <XCircle size={13} />
                  {t("reject")}
                </button>
