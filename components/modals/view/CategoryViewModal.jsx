    ...
    >
      {!category ? (
        ...
        <div dir={isArabic ? "rtl" : "ltr"} className="pb-4">
          <div className="flex gap-4 mb-4">
            {category.image && (
              <div className="flex-1 h-32 rounded-sm overflow-hidden border border-neutral-200 dark:border-neutral-800 relative">
                <span className="absolute top-0 left-0 bg-black/50 text-white text-[8px] px-2 py-1 uppercase tracking-widest font-bold z-10 backdrop-blur-sm">
                  {t("cover")}
                </span>
                <SafeImage
                  src={category.image}
          ...
          <Accordion title={t("basicInformation")} defaultOpen={true}>
            <RenderField label={t("name")} value={category.name} />
            <RenderField label={t("slug")} value={category.slug} />
            <RenderField label={t("totalProducts")} value={category.productCount} isBadge />
            <RenderField label={t("description")} value={category.description} isFullWidth />
          </Accordion>

          <Accordion title={t("visibilityAndFlags")}>
            <RenderField label={t("isActive")} value={category.isActive ? t("yes") : t("no")} isBadge />
            <RenderField label={t("isFeatured")} value={category.isFeatured ? t("yes") : t("no")} isBadge />
            <RenderField label={t("showOnWebsite")} value={category.showOnWebsite ? t("yes") : t("no")} isBadge />
            <RenderField label={t("showOnPOS")} value={category.showOnPOS ? t("yes") : t("no")} isBadge />
          </Accordion>

          <Accordion title={t("systemInfo")}>
            <RenderField label={t("createdAt")} value={new Date(category.createdAt).toLocaleString()} />
            <RenderField label={t("updatedAt")} value={new Date(category.updatedAt).toLocaleString()} />
            <RenderField label={t("createdBy")} value={category.createdBy?.name || category.createdBy} />
          </Accordion>
        </div>
      )}
    </BaseModal>
