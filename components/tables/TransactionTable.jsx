...
export const TransactionTable = ({ data, loading, onView, onReconcile, onReverse }) => {
  const columns = [
    ...
    },
    {
      header: "Type",
      render: (row) => (
        <span className={`text-[9px] font-black uppercase ${row.type === 'CREDIT' ? 'text-green-500' : 'text-red-500'}`}>
          {row.type}
        </span>
      )
    },
    ...
    {
      header: "Method",
      render: (row) => <span className="text-[9px] uppercase">{row.paymentMethod}</span>
    },
    {
      header: "Reconciled",
      render: (row) => (
