...
export const BaseTable = ({ columns, data, loading }) => {
  ...
  if (loading) return (
    <div className="p-8 text-center text-[10px] uppercase tracking-widest text-neutral-500">
      {t('loading')}
    </div>
  );
      ...
      <table className="w-full text-left border-collapse" dir={isArabic ? 'rtl' : 'ltr'}>
        <thead className="bg-neutral-50 dark:bg-[#0a0a0a] border-b border-neutral-200 dark:border-neutral-800">
          <tr>
            {columns.map((col, i) => (
              <th
                key={i}
                className={`p-4 text-[10px] uppercase tracking-[0.2em] font-black text-neutral-500 ${
        ...
        <tbody>
          {data.map((row, i) => (
            <tr
              key={row._id || i}
              className="border-b border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-[#0a0a0a]/50 transition-colors"
            >
              {columns.map((col, j) => (
                <td
                  key={j}
                  className={`p-4 text-[11px] font-medium text-black dark:text-white ${
                    isArabic ? 'text-right' : 'text-left'
                  }`}
                >
