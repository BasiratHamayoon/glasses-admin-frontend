...
export const AdminSidebar = ({
  ...
  const isSectionCollapsed = (idx) => {
    return collapsedSections[idx] === true;
  };
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto py-6 px-3 space-y-8 h-full [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-neutral-200 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-800 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-neutral-300 dark:hover:[&::-webkit-scrollbar-thumb]:bg-neutral-700 transition-colors">
        {navLinks.map((section, idx) => {
          const sectionCollapsed = isSectionCollapsed(idx);
          ...
          return (
            <div key={idx} className={isCollapsed ? "flex flex-col items-center" : ""}>
              {!isCollapsed ? (
                ...
                >
                  <h4
                    className="text-[9px] uppercase tracking-[0.3em] text-neutral-400 group-hover:text-neutral-600 dark:group-hover:text-neutral-300 transition-colors"
                  ...
                  <ChevronDown
                    size={12}
                    className={`text-neutral-400 group-hover:text-neutral-600 dark:group-hover:text-neutral-300 transition-all duration-200 ${
                      sectionCollapsed ? "rotate-0" : "rotate-180"
                    }`}
                  />
                </button>
              ) : (
                ...
                <div className="space-y-1 w-full">
                  {section.items.map(item => {
                    ...
                    const borderClass = isActive
                      ? `bg-[#E9B10C]/10 text-[#E9B10C] font-black ${
