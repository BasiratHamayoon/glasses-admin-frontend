...
export const ShopCard = ({ shop, onEdit, onDelete, onView }) => {
  ...
  return (
    <div className="bg-white dark:bg-[#111111] border border-neutral-200 dark:border-neutral-800 p-6 rounded-sm flex flex-col gap-5 group hover:border-[#E9B10C] transition-all duration-300 relative shadow-sm hover:shadow-md">
      <div
        className="flex justify-between items-center border-b border-neutral-100 dark:border-neutral-800 pb-3"
      ...
      <div dir={isArabic ? "rtl" : "ltr"} className="flex-1">
        <h3 className="text-lg font-black text-black dark:text-white mb-1 truncate">
          {shop.name}
        </h3>
        <p className="text-[11px] uppercase tracking-widest text-[#E9B10C] font-bold mb-4">
          {t(shop.shopType?.toLowerCase()) || shop.shopType}
        </p>
      ...
      <div
        className="flex items-center justify-between pt-4 border-t border-neutral-100 dark:border-neutral-800"
        dir={isArabic ? "rtl" : "ltr"}
      >
        <button
          onClick={onView}
        ...
        <div className="flex gap-4">
          <button
            onClick={onEdit}
            className="text-neutral-400 hover:text-[#E9B10C] transition-colors"
            title={t("edit")}
          >
            <Edit2 size={16} />
          </button>
