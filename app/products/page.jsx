...
export default function ProductsPage() {
  ...
  const currentLookupState = isLookupTab
    ? lookupStateMap[activePageTab] || {}
    : {};
  const currentLookupItems = Array.isArray(currentLookupState.items)
  ...
  const runProductFetch = useCallback(
    (page, filters) =>
      dispatch(fetchProducts(buildProductParams(filters, page))),
    [dispatch]
  );
  ...
  }, [productFilters.search, productFilters.status, productFilters.priceRange]);
  ...
  }, [lookupFilters.search, lookupFilters.isActive]);
  ...
  const confirmLookupDelete = async () => {
    ...
    try {
      await dispatch(
        config.actions.remove(lookupDeleteData.item._id)
      ).unwrap();
      toast.success(t("deletedSuccessfully"));
  ...
  return (
    <div className="space-y-6" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full border-b border-neutral-200 dark:border-neutral-800 pb-4">
          ...
          <button
            type="button"
            onClick={
              activePageTab === "products" ? openCreateProduct : openCreateLookup
            }
      ...
      <ConfirmationModal
        ...
        onClose={() =>
          setProductDeleteData({ ...productDeleteData, isOpen: false })
        }
      ...
      <ConfirmationModal
        ...
        onClose={() =>
          setLookupDeleteData({ ...lookupDeleteData, isOpen: false })
        }
