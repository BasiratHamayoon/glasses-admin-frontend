...
export default function InventoryPage() {
  ...
  const [webStockFilters, setWebStockFilters] = useState({ search: "", status: [] });
    ...
    <div className="space-y-6" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto scrollbar-hide">
          {activeTab === "shopStock" && (
            <button
              onClick={() => { setSelectedItem(null); setEditStockOpen(true); }}
              className="flex items-center justify-center gap-2 px-6 py-2.5 bg-black dark:bg-white text-white dark:text-black hover:bg-[#E9B10C] hover:text-black transition-colors rounded-sm shrink-0"
            >
          ...
          {activeTab === "webStock" && (
            <button
              onClick={() => { setSelectedItem(null); setEditWebStockOpen(true); }}
              className="flex items-center justify-center gap-2 px-6 py-2.5 bg-black dark:bg-white text-white dark:text-black hover:bg-[#E9B10C] hover:text-black transition-colors rounded-sm shrink-0"
            >
              <Plus size={14} strokeWidth={3} />
        ...
        {activeTab === "shopStock" && (
          <>
            <StockFilter
              ...
              onClear={() => { setStockFilters({ search: "", status: [] }); setCurrentPage(1); setTimeout(loadData, 100); }}
            />
            <StockTable
              data={stocks.items}
              loading={stocks.loading}
              onView={(item) => setViewStock({ isOpen: true, data: item, isWebsite: false })}
              onEdit={(item) => { setSelectedItem(item); setEditStockOpen(true); }}
            />
            <BasePagination
              currentPage={currentPage}
              totalPages={stocks.pagination?.totalPages || 1}
        ...
        {activeTab === "webStock" && (
          <>
            <StockFilter
              ...
              onApply={() => { setCurrentPage(1); loadData(); }}
              onClear={() => { setWebStockFilters({ search: "", status: [] }); setCurrentPage(1); setTimeout(loadData, 100); }}
            />
            <StockTable
              data={websiteStocks.items}
              loading={websiteStocks.loading}
              onView={(item) => setViewStock({ isOpen: true, data: item, isWebsite: true })}
              onEdit={(item) => { setSelectedItem(item); setEditWebStockOpen(true); }}
              isWebsite={true}
            />
            <BasePagination
              currentPage={currentPage}
              totalPages={websiteStocks.pagination?.totalPages || 1}
              onPageChange={setCurrentPage}
            />
          </>
        )}
      ...
      <StockModal
        ...
        onClose={() => { setEditStockOpen(false); loadData(); }}
        initialData={selectedItem}
      />
      <WebsiteStockModal
        isOpen={isEditWebStockOpen}
        onClose={() => { setEditWebStockOpen(false); loadData(); }}
        initialData={selectedItem}
      />
      <StockViewModal
        isOpen={viewStock.isOpen}
        onClose={() => setViewStock({ isOpen: false, data: null, isWebsite: false })}
        stock={viewStock.data}
        isWebsite={viewStock.isWebsite}
      />
    </div>
  );
}