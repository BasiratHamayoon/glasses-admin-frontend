...
export default function FinancePage() {
  ...
  const handleClearTxnFilters = () => {
    ...
    txnFiltersRef.current = initialTxnFilters;
    setCurrentPage(1);
    pageRef.current = 1;
    skipNextPageEffect.current = true;
    dispatch(fetchTransactions(buildTxnParams(initialTxnFilters, 1)));
  };

  const handleClearExpFilters = () => {
    setExpFilters(initialExpFilters);
    expFiltersRef.current = initialExpFilters;
    setCurrentPage(1);
    pageRef.current = 1;
    skipNextPageEffect.current = true;
    dispatch(fetchExpenses(buildExpParams(initialExpFilters, 1)));
  };
