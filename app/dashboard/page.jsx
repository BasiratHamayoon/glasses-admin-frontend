...
export default function DashboardPage() {
  ...
  const { stats, weeklySales, systemStatus, loading } = useSelector(
  ...
  const system = systemStatus?.systemStatus || systemStatus || {};

  if (loading && !Object.keys(statsData).length) return <PageSkeleton />;
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="flex justify-between items-center bg-neutral-50 dark:bg-[#0a0a0a] p-3 rounded-sm">
              <span className="text-[10px] uppercase font-bold text-neutral-500">
                {t("databaseStatus") || "DB Status"}
              </span>
              <span
                className={`text-[10px] font-black flex items-center gap-1 ${
                  system?.database?.isConnected ? "text-green-500" : "text-red-500"
                }`}
              >
            ...
            <div className="flex justify-between items-center bg-neutral-50 dark:bg-[#0a0a0a] p-3 rounded-sm">
              ...
              <span className="text-[10px] font-black">
                {system?.server?.memoryUsage?.rss || "N/A"}
              </span>
            </div>
          </div>
        </div>
