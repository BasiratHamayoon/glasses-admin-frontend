...
export default function ShopsPage() {
  ...
  const [shopFilters, setShopFilters] = useState({ search: "", status: [], shopType: [] });
  ...
  const handleViewShop = async (shop) => {
    try {
      ...
      toast.error(t("error") || "Failed to load details");
    }
  };
  ...
  return (
    <div className="space-y-6" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full border-b border-neutral-200 dark:border-neutral-800 pb-4">
        ...
        <button
          onClick={() => { setSelectedItem(null); setEditModalOpen(true); }}
          className="px-6 py-2.5 bg-black dark:bg-white text-white dark:text-black hover:bg-[#E9B10C] hover:text-black transition-colors text-[10px] uppercase font-black rounded-sm flex items-center justify-center gap-2 w-full sm:w-auto shadow-sm"
        >
      ...
      <div className="bg-white dark:bg-[#111111] border border-neutral-200 dark:border-neutral-800 p-4 sm:p-6 rounded-sm min-h-[400px]">
        <ShopFilter
          ...
          onClear={() => { setShopFilters({ search: "", status: [], shopType: [] }); setCurrentPage(1); setTimeout(loadData, 100); }}
        />
      ...
      <ShopModal
        ...
        onClose={() => { setEditModalOpen(false); loadData(); }}
      ...
      <ConfirmationModal
        ...
        message={t("deleteWarning") || "Are you sure you want to delete this shop?"}
      />
    </div>
  );
}